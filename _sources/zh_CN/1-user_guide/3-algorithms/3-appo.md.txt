...
APPO 顶层 CLI 会路由到 `scripts/train_appo.py`。常规训练用
`uv run train --algo appo ...`，checkpoint 回放用 `uv run eval --algo appo ...`。
...
uv run eval --algo appo --task go2_joystick_flat --sim mujoco \
