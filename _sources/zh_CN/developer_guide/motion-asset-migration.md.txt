...
本地目录 `src/unilab/assets/motions/g1/` 保留，作为下载落盘位置。
...
Git 历史覆写（`git filter-repo`）完成后，所有协作者需要重新同步：
...
**注意**：历史覆写后，旧的 commit hash 将全部失效。请确保在覆写前推送所有本地提交。

## 架构说明

- 资产解析模块：`src/unilab/assets/hub.py`
