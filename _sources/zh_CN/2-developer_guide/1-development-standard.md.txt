...
合成顺序: 顶层 CLI `--algo` / `--task` / `--sim` 选择算法配置与
owner YAML，再叠加普通 Hydra override。
...
- `training.sim_backend` 是 task owner YAML 的身份字段，不是独立的 backend switch；切换后端必须使用顶层 CLI 的 `--sim <backend>` 重新选择 owner YAML，不能只 override `training.sim_backend`
- reward 必须显式注入
...
- {doc}`Domain Randomization Contract <4-domain-randomization-contract>`

协作与交付流程要求见 [Collaboration](2-collaboration.md)。
...
- Next: [Domain Randomization Contract](4-domain-randomization-contract.md)
